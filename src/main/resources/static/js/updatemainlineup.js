        ...
        if (xhr.status==200) {
            ...
            for(var i = 0;i<data.length;i++){
                if(data[i].status=='main'){
                    main.push(data[i].player.id)
                }else {
                    resever.push(data[i].player.id)
                }
            }
            getPlayerResponse()
        }
...
function getPlayerByIdTeam(playerList){
    ...
    for(var i= 0 ;i<playerList.length;i++){
        ...
        console.log(main)
        if(main.includes(parseInt(playerList[i].id))){
...
button.addEventListener("click", function() {
    ...
    if(count!=11){
        ...
        localStorage.setItem("reserve",resever);
        deleteLineup();
        postMainLineup(list);
    }
});
...
// Phân tích URL và trích xuất phần số
    ...
    xhr.onload = function () {
        if (xhr.status==201) {
            ...
            window.location = "/updatereservelineup/"+number2+"/"+number;
        } else {
            // Xử lý khi request không thành công
            console.error('Có lỗi xảy ra');
            console.error(xhr.statusText);
